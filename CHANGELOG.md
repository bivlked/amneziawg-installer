...
- `tests/test_boot_critical_packages.bats` (16 проверок): проверяется порядок, а не только наличие. `apt-mark manual` обязан стоять до обновления, а сверка после него и до перезагрузки; одного наличия было бы мало, потому что прежний `apt-mark hold` тоже присутствовал и всё равно не помог. Функциональная часть поднимает реальные блоки из поставляемого установщика, а не повторяет их логику. Набор проверен на девяти мутантах: удаление любой из двух защит, перенос сверки за перезагрузку, замена её `die` на предупреждение и выпадение пакета из списка делают набор красным
