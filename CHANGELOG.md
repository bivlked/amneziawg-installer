...
- **Ссылка `vpn://` этот формат не портила и не портит.** Внутрь неё клиентский конфиг вкладывается из файла как есть, поэтому у слипшейся ссылки причина всегда одна: `.conf`, испорченный прежним `regen`. Отдельное структурное поле `allowed_ips` собирается из компактной формы намеренно, потому что оно превращается в JSON-массив, и пробел уехал бы внутрь его элементов. При подготовке этого выпуска нормализация была по ошибке заведена и туда: на стенде это дало ведущий пробел у 33 записей из 34. Правка отменена, а в коде и в тестах теперь закреплено, почему в этом месте пробелы срезаются

### Добавлено

- `tests/test_allowedips_dns_spacing.bats` (17 проверок): канонический разделитель, восстановление уже слипшегося списка, рваные пробелы, лишние запятые, CRLF, защита от glob-раскрытия, идемпотентность, неприкосновенность одиночных значений, три функциональных прогона `regenerate_client`, две проверки `modify` и охрана компактной формы в сборщике `vpn://`. Заодно исправлено ожидание в `tests/test_issue170_route_mode_change.bats`: оно закрепляло слипшийся список как норму
