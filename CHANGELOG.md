...
**v5.27.1** - списки `AllowedIPs` и `DNS` больше не теряют пробелы после запятых, а конфиги, испорченные прежними версиями, чинятся повторным `regen`.

### Исправлено

- **`regen` и ссылка `vpn://` молча слепляли списки.** Установщик пишет `AllowedIPs` и `DNS` через запятую с пробелом, а при чтении этих значений пробелы срезались заодно с символом возврата каретки: в `regenerate_client` через `tr -d '[:space:]'`, в сборщике `vpn://` через `tr -d ' \r'`. Прочитанное значение записывалось обратно, поэтому первый же `regen` превращал `1.0.0.0/8, 2.0.0.0/7` в `1.0.0.0/8,2.0.0.0/7`, и дальше эта форма переживала все последующие перевыпуски. Глазом разница не видна, а в режиме по умолчанию «Список Amnezia + DNS» речь идёт о 34 записях. Теперь оба пути приводят список к каноническому виду `a, b, c` вместо вырезания пробелов, поэтому **повторный `regen` не только сохраняет формат, но и восстанавливает его в конфигах, испорченных прежними версиями**. Символ возврата каретки по-прежнему удаляется: на конфигах с окончаниями строк CRLF он иначе прилипал к последнему элементу списка и ломал JSON внутри ссылки. Спасибо `@humowns` за находку и за проверку маршрутов на своей стороне ([обсуждение #38](https://github.com/bivlked/amneziawg-installer/discussions/38))
- **`manage modify` записывал список в том виде, в каком его набрали.** Значение проходило поэлементную проверку, но в файл попадало дословно, поэтому `modify ИМЯ AllowedIPs "a,b"` оставлял в конфиге второй, слипшийся вариант того же списка. После исправления выше он бы там и законсервировался, так что `AllowedIPs` и `DNS` нормализуются теперь и здесь: все три пути (`add`, `regen`, `modify`) дают одинаковый формат
- **Разбор `DNS` при сборке ссылки оставлял ведущий пробел.** Второй адрес выделялся отрезанием по первой запятой, что при каноническом разделителе `, ` дало бы значение с пробелом прямо в поле JSON. Разбор переписан поэлементно, с обрезкой краёв у каждого элемента

### Добавлено

- `tests/test_allowedips_dns_spacing.bats` (16 проверок): канонический разделитель, восстановление уже слипшегося списка, рваные пробелы, лишние запятые, CRLF, защита от glob-раскрытия, идемпотентность, неприкосновенность одиночных значений, три функциональных прогона `regenerate_client` и сверка RU/EN. Заодно исправлено ожидание в `tests/test_issue170_route_mode_change.bats`: оно закрепляло слипшийся список как норму
