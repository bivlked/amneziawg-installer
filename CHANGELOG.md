...
- **Установщик говорит вслух, что обновилось не всё.** Обратная сторона перехода на `apt-get upgrade`: пакет, обновление которого потребовало бы удаления соседа, теперь остаётся на прежней версии, а apt возвращает при этом ноль. Для загрузки сервера это безопасно и ровно этого мы и добивались, но молчать об этом нельзя: `full-upgrade` откладывать пакеты не умел вовсе, так что без отдельной строки диагностируемость стала бы хуже, чем была до фикса. После обновления печатается список задержавшихся пакетов. Формулировка намеренно не называет одну причину: удержать пакет может и наш новый режим, и поэтапная раскатка выпуска у Ubuntu, а различить их этой строкой нельзя
