...
- `tests/test_boot_critical_packages.bats` (27 проверок): проверяется порядок, а не только наличие. `apt-mark manual` обязан стоять до обновления, а сверка после него и до перезагрузки; одного наличия было бы мало, потому что прежний `apt-mark hold` тоже присутствовал и всё равно не помог. Функциональная часть поднимает реальные блоки из поставляемого установщика, а не повторяет их логику. Набор проверялся подстановкой сломанных версий установщика: удаление любой из трёх защит, перенос проверки с последнего места, замена её `die` на предупреждение, выпадение пакета из списка и ослабление разбора статуса dpkg делают набор красным
