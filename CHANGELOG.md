...
**v5.21.1** - надёжный разбор порта в `check --json`: вывод остаётся разбираемым при любом содержимом конфига.

### Исправлено

- **`check --json` больше не ломается на нечисловом `AWG_PORT`**: значение из `awgsetup_cfg.init` подставлялось в поле `port.number` без проверки, и после неудачной ручной правки конфига вывод переставал быть валидным JSON - это нарушало обещание v5.21.0 про ровно один разбираемый документ на любом пути выхода. Теперь порт нормализуется при чтении: число из диапазона 1-65535 проходит как есть, всё остальное становится `0` - значением, которое команда уже использует в смысле «порт не определён». Попутно закрыт побочный эффект: то же непроверенное значение попадало в арифметическое сравнение `[[ -eq ]]`, где bash выполняет подстановку команд; файл конфига доступен только root, так что подсунуть туда чужое нельзя, но полагаться на права там, где хватает одной проверки, незачем
