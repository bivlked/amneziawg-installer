...
- **Guard смены подсети.** Переустановка (`--force`) с другой подсетью туннеля прерывается, если в awg0.conf уже есть пиры: их адреса выданы в старой подсети (старые IPv4 могут выпасть из нового диапазона, IPv6-суффиксы — столкнуться). Перед сменой подсети удалите клиентов либо выполните `--uninstall` и чистую установку.
...
- Определение сетевого интерфейса больше не обрывает установку на хостах, где зонд к `1.1.1.1` не возвращает интерфейс - провайдер блокирует или null-route'ит этот адрес, policy-routing либо IPv6-only egress (наблюдалось на Ubuntu 26.04 / Timeweb). `get_main_nic` теперь пробует цепочку: `ip route get`, дефолтный IPv4-маршрут, первый глобальный IPv4-интерфейс, дефолтный IPv6-маршрут; при полной неудаче печатает список доступных интерфейсов и подсказку. Интерфейс можно задать вручную через `AWG_MAIN_NIC=<iface>`. Раньше шаг 6 обрывался ошибкой "Не удалось определить сетевой интерфейс" (#166)
