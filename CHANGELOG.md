...
- То же непроверенное значение попадало в арифметическое сравнение `[[ -eq ]]`, где bash выполняет подстановку команд, и в regex проверки правил UFW в `diagnose`. Конфиг доступен только root, поэтому чужого туда не подложить, но полагаться на права там, где хватает одной проверки, незачем
