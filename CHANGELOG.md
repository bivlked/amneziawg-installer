...
**v5.16.0** - комплексный аудит кода: безопасность, надёжность файловых операций и точность валидации. Релиз закрывает находки полного ревью всех шести скриптов: устранено окно потери пиров при `--force` reinstall, секреты убраны из argv, GPG-ключ PPA закреплён по полному fingerprint, валидатор AWG-параметров проверяет непересечение H1-H4, а истёкшие orphan-клиенты больше не зацикливают cron. Поведение установки по умолчанию не меняется. Матрица поддержки прежняя: Ubuntu 24.04 / 25.10 / 26.04, Debian 12 / 13, x86_64 + ARM.
