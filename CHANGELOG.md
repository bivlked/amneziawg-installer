...
**v5.28.1** - обновление системы на первом шаге больше не может удалить установленный пакет, а восстановление собирает связанные пакеты одной транзакцией.

### Исправлено

- **Обновление системы на первом шаге больше не вправе удалять установленные пакеты.** Защита из v5.28.0 ловила потерю уже после факта и пыталась вернуть пакеты на место. На двух серверах из issue #223 это не сработало: `apt full-upgrade` отработал успешно и всё равно снёс шесть защищённых пакетов из восьми, поскольку признак manual удаление не запрещает, а лишь снимает статус «больше не требуется». Дело в самой команде: `full-upgrade` по определению вправе выбрать удаление пакета, если так проще разрешить зависимости. Теперь на первом шаге выполняется `apt-get upgrade --with-new-pkgs`, у которой такого права нет вовсе: пакет, который нельзя обновить без удаления соседа, просто остаётся на прежней версии. Флаг `--with-new-pkgs` сохраняет единственное, ради чего здесь нужен был `full-upgrade`: новое ядро приезжает пакетом с новым именем (`linux-image-6.8.0-NNN-generic`), а обычный `upgrade` новые имена ставить отказывается. Размен осознанный: серверу под VPN нужно не самое свежее окружение, а гарантия, что он загрузится. Проверка перед перезагрузкой остаётся вторым рубежом. Затронуты обе версии установщика
- **Восстановление пакетов идёт одной транзакцией, а поштучный проход стал запасным вариантом.** Пакеты, связанные жёстким условием на версию друг друга, поодиночке не восстанавливаются в принципе: каждая отдельная команда обязана оставить остальные имена как есть и потому не может подтянуть соседа нужной версии. В issue #223 на этом восстановление и встало, выдав пять отказов подряд вида `udev : Breaks: systemd (< 255.4-1ubuntu8.17) but 255.4-1ubuntu8.12 is to be installed`, при том что вместе те же имена сходятся. Теперь сначала выполняется одна транзакция со всеми потерянными именами, и лишь затем, для того что всё ещё отсутствует, поштучный проход. Он сохранён намеренно: одно имя без кандидата на установку отменяет транзакцию целиком, и тогда не восстановится ничего, включая пакеты, которые ставятся прекрасно
- **Фатальное сообщение о неудачном обновлении больше не называет причину, которой не наблюдало.** Прежняя редакция безусловно винила занятый dpkg-lock, включая случай, когда проверка `fuser` двумя строками выше ничего не нашла. В issue #223 настоящей причиной был отказ резолвера (`netplan.io : Depends: udev but it is not going to be installed`), и он был на экране, а сообщение уводило разбор в сторону. Теперь версия про блокировку печатается только при живом держателе, причём замер берётся заново непосредственно перед сообщением: между первой попыткой и этим местом выполняются `dpkg --configure -a` и вторая попытка, за которые найденный процесс мог давно завершиться. Если блокировки нет, показывается настоящий ответ apt, добытый холостым прогоном под `timeout`. Когда холостой прогон проходит успешно, его вывод не выдаётся за причину отказа: печатается прямое указание, что зависимости сходятся и дело не в них
