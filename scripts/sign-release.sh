...
# Проверка гоняется ВСЕГДА, в том числе после частичного отказа. Прежде выход
# происходил раньше, и человек знал только число, но не знал, какие файлы уже
# несут новый тег, а какие остались от прежнего релиза - а именно эта смесь и
# опасна: файлов по-прежнему шесть, счётчик ничего не замечает.
echo
verify_rc=0
bash "$SCRIPT_DIR/verify-signatures.sh" "$TAG" || verify_rc=$?

if [[ "$failed" -ne 0 ]]; then
    echo "ОТКАЗ: подписаны не все файлы (см. состояние подписей выше)" >&2
    exit 1
fi
exit "$verify_rc"
