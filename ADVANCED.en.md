...
- `check.protocol` is the protocol generation of the installation as a string (`"2.0"` or `"3.1"`), read from the `AWG_PROTOCOL` marker in `awgsetup_cfg.init` and not from the module version: a third-line module runs a second-line configuration as a matter of course. An installation without the marker is `"2.0"`. `check.protocol_error` is normally `null`; the value `"unreadable"` means the marker is broken (a value outside the set, an empty field, two marker lines, an unreadable file): `protocol` is then `null`, `ok=false` and the exit code is 1. Both fields are always present.
