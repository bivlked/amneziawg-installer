...
- `check.protocol` is the protocol generation of the installation as a string (`"2.0"` or `"3.1"`), read from the `AWG_PROTOCOL` marker in `awgsetup_cfg.init` and not from the module version: a third-line module runs a second-line configuration as a matter of course. An installation without the marker is `"2.0"`. `check.protocol_error` is normally `null`; the value `"unreadable"` means the marker is broken (a value outside the set, an empty field, two marker lines, a file that cannot be read): `protocol` is then `null`, `ok=false` and the exit code is 1. Both fields are present in every answer of the check itself. If `awgsetup_cfg.init` is missing or is not a regular file, `check` stops before the check with the generic error envelope (`ok`, `error`, `rc`), which does not carry them.
