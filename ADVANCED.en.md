...
> Alongside sits `<name>.png` - the QR of `.conf` for AmneziaWG apps that import by QR (AmneziaWG for Windows and the other AmneziaWG apps; `awg-quick` takes the same `.conf` as a file); a plain WireGuard client cannot import it. The two formats target different clients: the Amnezia VPN app scans `.vpnuri.png`, AmneziaWG clients scan `<name>.png`. Do not mix them up.
