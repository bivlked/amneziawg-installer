...
⚠️ **Одно исключение, о котором стоит помнить.** Флаги `--mobile`, `--preset`, `--jc`, `--jmin` и `--jmax` перегенерируют весь набор параметров обфускации, а `--port` (и `--mobile`, который ставит 443) и `--endpoint` меняют порт или адрес в `Endpoint`. В обоих случаях все выданные раньше конфигурации перестают подключаться, пока вы не перевыпустите их через `regen` и не раздадите заново. Если вы просто обновляетесь, эти флаги не передавайте.
