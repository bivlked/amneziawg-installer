...
- Ubuntu 24.04 LTS, Ubuntu 26.04, Debian 12 (bookworm), Debian 13 (trixie). Ubuntu 25.10 тоже работает, но подробности ниже.
...
- ⚠️ **Ubuntu 25.10** формально поддерживается, но у самой Ubuntu она уже снята с поддержки (EOL). Для нового сервера смысла её брать нет: берите 24.04 LTS или 26.04. Если 25.10 у вас уже стоит, установщик с ней работает так же, через переключение на `noble`.
- **Debian 12** (bookworm) и **Debian 13** (trixie) поддержаны полностью, кодовое имя отображается на `focal` и `noble` соответственно.
