...
Два вопроса легко пропустить. В начале скрипт покажет пакеты, которые собирается удалить (snapd, unattended-upgrades и другие), и спросит разрешения; оставить их - ответ `n` или флаг `--keep-packages`. В третьем запуске он спросит «Включить UFW? [y/N]»: ответьте `y`, по Enter сервер останется без фаервола.
...
⚠️ **Одно исключение, о котором стоит помнить.** Флаги `--mobile`, `--preset`, `--jc`, `--jmin` и `--jmax` перегенерируют весь набор параметров обфускации, а `--port` (и `--mobile`, который ставит 443) меняет порт в `Endpoint`. В обоих случаях все выданные раньше конфигурации перестают подключаться, пока вы не перевыпустите их через `regen` и не раздадите заново. Если вы просто обновляетесь, эти флаги не передавайте.
