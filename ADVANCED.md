...
- `check.port.number` - всегда целое. `0` читайте как «порт не определён». Причин три: настройки нет в `awgsetup_cfg.init`, там лежит не число, или число вне диапазона 1-65535 (файл правится руками). Первый случай - предупреждение, два остальных считаются повреждённым конфигом и дают `ok=false` с кодом возврата 1. Судить о живом порте по одному этому полю нельзя - смотрите вместе с `port.listening`.
