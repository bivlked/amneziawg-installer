...
2. Экспортируйте нужный: `export AWG_MAIN_NIC=ens3`
3. Перезапустите: `bash install_amneziawg.sh` - значение подхватится на шаге 6

Значение проверяется (существующий интерфейс без спецсимволов), поэтому опечатка просто игнорируется и снова срабатывает авто-определение.
</details>
