...
- `list.expires_at` - unix-время числом у клиента со сроком либо `null` у бессрочного; каноническая десятичная запись не длиннее 10 знаков. `list.expires_at_error` - в норме `null`, значение `"unreadable"` означает, что метка срока на сервере есть, а значения из неё получить не удалось (пустой файл, повреждённое содержимое, каталог на месте файла, отказ по правам). Оба поля присутствуют в каждой записи всегда; подробности - в разделе [Временные клиенты](#expires-adv).
...
> Значение `expires_at` - каноническая десятичная запись не длиннее 10 знаков. Ведущий ноль,
...
> повреждённое содержимое, каталог на месте файла и файл, который не читается по правам. Один
