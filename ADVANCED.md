...
2.  **Режим 2: Список Amnezia + DNS**
    ...
    * IPv6: без `--allow-ipv6-tunnel` к этому списку тоже дописывается `::/0`, так что вне туннеля остаётся только локальная сеть по IPv4, а локальный IPv6 при включённом VPN уходит в туннель. Нужен живой IPv6 устройства - смотрите [IPv6 устройства, `::/0` и локальная сеть](#client-ipv6-adv).
...
- **полный туннель** (режимы 1 и 2) без `--allow-ipv6-tunnel` получает `::/0`. IPv6 устройства уходит в туннель, дальше не идёт, и устройство переходит на IPv4. Пока VPN включён, не открываются ресурсы, доступные только по IPv6, и недоступен локальный IPv6 домашней сети;
...
`manage regen` дописывает `::/0` и в уже выданные профили полного туннеля, если в их `AllowedIPs` нет ни одного IPv6-маршрута, в том числе в списки, поправленные через `modify`. Как это выглядит со стороны сайтов и как проверить с устройства - в разделе [Что видно сайту при делении трафика по адресу назначения](#split-detect-adv).
