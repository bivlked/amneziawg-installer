...
Раз ванильный WireGuard работает, сеть, порт и фаервол можно вычеркнуть. Разница между ним и AmneziaWG ровно одна - слой обфускации: `Jc`/`Jmin`/`Jmax`, `S1`-`S4`, `H1`-`H4` и, в версии 2.0, `I1`-`I5`. Если хоть один из этих параметров на сервере и клиенте не совпадает байт-в-байт, сервер не может разобрать входящий handshake и молча отбрасывает его - снаружи это выглядит как «пакеты идут, но не обрабатываются».
