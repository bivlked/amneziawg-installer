...
2.  **Режим 2: Список Amnezia + DNS**
    ...
    * IPv6: к этому списку тоже дописывается `::/0`, так что вне туннеля остаётся только локальная сеть по IPv4, а локальный IPv6 при включённом VPN уходит в туннель. Нужен живой IPv6 устройства - смотрите [IPv6 устройства, `::/0` и локальная сеть](#client-ipv6-adv).
...
- **полный туннель** (режимы 1 и 2) получает `::/0`. IPv6 устройства уходит в туннель, дальше не идёт, и устройство переходит на IPv4. Пока VPN включён, не открываются ресурсы, доступные только по IPv6, и недоступен локальный IPv6 домашней сети;
...
- **IPv6 внутри туннеля** включается отдельно, флагом `--allow-ipv6-tunnel`, см. [IPv6 dual-stack в туннеле](#ipv6-tunnel-adv).
