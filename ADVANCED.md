...
**DNS новых клиентов** задаётся строкой в `/root/awg/awgsetup_cfg.init`:

```bash
echo "export CLIENT_DNS='10.9.9.1'" | sudo tee -a /root/awg/awgsetup_cfg.init
```

Значение - IP-адреса через запятую. Его получают клиенты, созданные после правки (`manage add`), и клиент, чей конфиг `regen` восстанавливает с нуля. Уже выданные конфиги не меняются: `regen` сохраняет их DNS, поменять его можно командой `modify <имя> DNS ...`. Невалидное значение молча не подменяется: `add` откажет и напишет, что не так. Переустановка с `--force` строку сохраняет. Без неё новые клиенты получают `1.1.1.1, 1.0.0.1`.
...
4. Готовый пакет можно не брать вовсе: с флагом `--no-prebuilt` модуль сразу собирается через DKMS. Готовые пакеты собирает CI проекта, а установщик сверяет их только с файлом SHA256 из того же релиза; подписи у них нет.
