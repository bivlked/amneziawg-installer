...
**Машиночитаемо:** `list --json` возвращает в каждой записи поле `expires_at` - unix-время
числом либо `null` у бессрочного клиента. Семантика та же, что у `add --json` в `results[]`,
поэтому внешнему потребителю (GUI, бот, мониторинг) не нужно читать `/root/awg/expiry/` по SSH.
...
```json
[
  {"name":"guest","ip":"10.9.9.4","client_ipv6":"","status":"Активен","status_code":"active","expires_at":1750000000,"expires_at_error":null},
  {"name":"pc","ip":"10.9.9.3","client_ipv6":"","status":"Недавно","status_code":"recent","expires_at":null,"expires_at_error":null}
]
```

> Рядом идёт признак `expires_at_error`. Он присутствует в каждой записи и в норме равен `null`;
> значение `"unreadable"` означает, что метка срока на сервере есть, но не разбирается (файл
> повреждён, правили руками). Один только `expires_at: null` в этом случае читался бы как
> «клиент бессрочный по замыслу», а на деле срок кому-то задавали и он молча не работает:
> проверка истечения такую метку пропускает с предупреждением в журнал и клиента не удаляет.
> Признак нужен, чтобы это состояние было видно и его починили.
