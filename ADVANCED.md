...
> **`--allowed-ips=СПИСОК`** - индивидуальные маршруты клиента сразу при создании, без последующего `modify` (Issue #253). Список CIDR IPv4/IPv6 через запятую: `add phone --allowed-ips="10.0.0.0/8, 192.168.0.0/16"`. Валидируется и нормализуется до создания первого клиента (невалидное значение рушит команду, ничего не создав), применяется ко всем именам batch-а - как `--expires`. Значение проходит те же правила, что и глобальный режим: полный туннель четвёртой версии получает `::/0` рядом (iOS-правило), раздельный список не получает никогда, а список с явными IPv6-токенами (например `0.0.0.0/0, ::/0`) записывается как есть. Без флага - глобальный режим маршрутизации сервера, как раньше. Существующим клиентам маршруты по-прежнему меняет `modify`, а `regen` сохраняет такие индивидуальные списки.
