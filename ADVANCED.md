...
🔴 **`RandomTrailers` включать не следует, пока не выйдет исправление.** Сам по себе параметр
бесплатен: на голом конфиге, без пакетов-пустышек, без `S`-паддинга и без маскировочных пакетов,
он даёт те же **11.94 МБ/с**, что и без него. Беда возникает в СОЧЕТАНИИ с полным набором
обфускации - там пропускная способность падает до **0.09-0.14 МБ/с**, то есть примерно в 125 раз.

Деградация нарастает с числом задействованных групп параметров: `S1`-`S3` вместе с `H1`-`H4`
дают 5.60 МБ/с, добавление `S4` роняет до 0.11. По отдельности ни одна группа с флагом не
конфликтует: `S4`, `S1`-`S3`, `H1`-`H4`, `Jc` и `I1` поодиночке дают 11.7-11.9 МБ/с.

Что это дефект реализации, а не свойство маршрута, показывает замер в локальной сети без выхода
в интернет: **44.22 МБ/с без параметра против 0.09 с ним**. Поэтому объяснять падение потерями
пакетов в пути неверно - потери здесь следствие, а не причина.
...
- **полный туннель без `--allow-ipv6-tunnel`** - сюда попадают и режим «Весь трафик» (`--route-all`), и дефолтный режим «Amnezia»: клиент получает `::/0` рядом с IPv4-маршрутами, и IPv6 устройства уходит в туннель. Сам туннель IPv6 не несёт, поэтому дальше такой трафик не идёт - но и мимо VPN не утекает. Обратная сторона: пока VPN включён, ресурсы, доступные ТОЛЬКО по IPv6, недоступны, и локальный IPv6 домашней сети тоже уходит в туннель (по IPv4 локальная сеть остаётся доступной);
...
> **IPv6 в IPv4-only режиме (по умолчанию).** Когда туннель работает только по IPv4, IPv6-трафик вашего устройства идёт напрямую, мимо VPN, - так и задумано: IPv4-only туннель по определению не несёт IPv6, и сервер на это не влияет (это свойство режима, а не утечка серверной стороны). Одна оговорка: сказанное верно для раздельной маршрутизации (режим 3, свой список сетей). В полном туннеле - а это и `--route-all`, и дефолтный режим «Amnezia» - клиент получает `::/0` рядом с IPv4-маршрутами, поэтому IPv6 устройства уходит в туннель и там глохнет, а не идёт напрямую - разбор в разделе [Что видно сайту при делении трафика по адресу назначения](#split-detect-adv). Нужен IPv6 в туннеле - включите `--allow-ipv6-tunnel` (ниже). Нужна, наоборот, гарантия, что ничего не идёт мимо VPN, - отключите IPv6 на самом устройстве: серверный фильтр тут не поможет, потому что прямой IPv6-трафик до сервера просто не доходит.
...
>
> 🔴 **Префикс `<r 2>` в этом паттерне обязателен, копировать блок без него нельзя.** Два случайных байта - это идентификатор транзакции DNS, с которого начинается любой пакет протокола. Без них всё содержимое сдвигается на два байта, и разбор ломается целиком: поле идентификатора получает `0x8580`, флаги становятся `0x0001`, то есть пакет объявляет себя запросом вместо ответа, счётчик дополнительных записей вырастает до 1641, а первая метка имени заявляет 99 байт при 29 оставшихся. Как DNS такой пакет не разбирается вовсе. Вторая причина не менее важна: без случайного префикса все 42 байта постоянны, и CPS-пакет становится байт-в-байт одинаковым у всех, кто скопировал рецепт отсюда - то есть готовым признаком для поиска.
