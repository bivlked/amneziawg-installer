...
- [⚙️ Детали конфигурации клиента](#config-details-adv)
  ...
  - [IPv6 устройства, `::/0` и локальная сеть](#client-ipv6-adv)
  ...
  - [DNS](#dns-adv)
  - [Свой DNS-сервер для клиентов](#own-dns-adv)
...
2.  **Режим 2: Список Amnezia + DNS**
    ...
    * IPv6: без `--allow-ipv6-tunnel` к этому списку дописывается `2000::/3` (весь глобальный IPv6), а в `Address` клиента служебный адрес вида `fddd:2c4:2c4:ffff::a09:905/128`. IPv6 устройства в интернет уходит в туннель и там гаснет, а локальная сеть остаётся вне туннеля целиком, и по IPv4, и по IPv6. Не `::/0` потому, что с ним клиент AmneziaWG для Windows включает блокировку всего трафика мимо туннеля, и локальная сеть пропадает. Нужен живой IPv6 устройства - смотрите [IPv6 устройства, `::/0` и локальная сеть](#client-ipv6-adv).

3.  **Режим 3: Пользовательский (Split-Tunneling)**
    * Только трафик к указанным сетям -> VPN.
    * Пример: `192.168.1.0/24,10.50.0.0/16`
    * `::/0` не дописывается, пока список не покрывает весь публичный IPv4: IPv6 устройства идёт мимо туннеля напрямую.
...
### IPv6 устройства, `::/0` и локальная сеть

`--disallow-ipv6` и ключ `DISABLE_IPV6` в `awgsetup_cfg.init` выключают IPv6 на самом сервере (sysctl хоста). IPv6 вашего устройства они не трогают и не решают, попадёт ли в клиентский `AllowedIPs` маршрут `::/0`. Это решает режим маршрутизации:

- **полный туннель** без `--allow-ipv6-tunnel`: режим 1 получает `::/0`, режим 2 получает `2000::/3` и служебный IPv6-адрес. IPv6 устройства уходит в туннель, дальше не идёт, и устройство переходит на IPv4. Пока VPN включён, не открываются ресурсы, доступные только по IPv6; в режиме 1 недоступен и локальный IPv6 домашней сети, в режиме 2 он остаётся доступным;
- **свой список сетей** (режим 3, `--route-custom`) `::/0` не получает, если не покрывает весь публичный IPv4. IPv6 устройства идёт мимо туннеля со своим адресом;
- **с `--allow-ipv6-tunnel`** правила другие: полный туннель получает `::/0`, только если у сервера есть нативный IPv6, и тогда IPv6 устройства идёт через VPN. Без нативного IPv6 клиенту достаётся лишь туннельная подсеть, и глобальный IPv6 устройства снова идёт мимо туннеля. Подробно - [IPv6 dual-stack в туннеле](#ipv6-tunnel-adv).

`manage regen` дописывает IPv6-маршрут и в уже выданные профили полного туннеля, если в их `AllowedIPs` нет ни одного IPv6-маршрута, в том числе в списки, поправленные через `modify`, а профилям режима 2, выданным с `::/0`, заменяет его на `2000::/3`. Как это выглядит со стороны сайтов и как проверить с устройства - в разделе [Что видно сайту при делении трафика по адресу назначения](#split-detect-adv).
...
**IPv6 - отдельный уровень, и его легко перепутать.** В гайдах фраза «IPv6 выключен, установщик так и ставит по умолчанию» означает IPv6 **на сервере** (sysctl хоста). IPv6 **на вашем устройстве** установщик не трогает вовсе, а деление трафика к нему не применяется: и список российских сетей у каскада, и BGP-фид у схемы с WARP содержат только IPv4 (в снимке `cascade/ru.zone` IPv6-записей нет ни одной). Пойдёт ли IPv6 устройства мимо туннеля, решает не схема, а то, попал ли в `AllowedIPs` IPv6-маршрут (`::/0` или `2000::/3`):

- **полный туннель без `--allow-ipv6-tunnel`** - сюда попадают и дефолтный режим «Весь трафик» (`--route-all`), и режим «Amnezia»: клиент получает рядом с IPv4-маршрутами IPv6-маршрут (`::/0` в режиме «Весь трафик», `2000::/3` со служебным адресом в режиме «Amnezia»), и IPv6 устройства уходит в туннель. Сам туннель IPv6 не несёт, поэтому дальше такой трафик не идёт - но и мимо VPN не утекает. Обратная сторона: пока VPN включён, ресурсы, доступные ТОЛЬКО по IPv6, недоступны, а в режиме «Весь трафик» и локальный IPv6 домашней сети уходит в туннель (в режиме «Amnezia» локальная сеть остаётся доступной и по IPv4, и по IPv6);
- **раздельная маршрутизация** (режим 3, свой список сетей через `--route-custom`): `::/0` не добавляется, пока список не покрывает весь публичный IPv4, иначе сломалось бы само деление. IPv6 устройства идёт мимо туннеля со своим настоящим адресом, и тогда сайту не нужны никакие скрипты - он видит этот адрес напрямую;
...
> **IPv6 в IPv4-only режиме (по умолчанию).** Когда туннель работает только по IPv4, IPv6-трафик вашего устройства идёт напрямую, мимо VPN, - так и задумано: IPv4-only туннель по определению не несёт IPv6, и сервер на это не влияет (это свойство режима, а не утечка серверной стороны). Одна оговорка: сказанное верно для раздельной маршрутизации (режим 3, свой список сетей). В полном туннеле - а это и дефолтный `--route-all`, и режим «Amnezia» - клиент получает IPv6-маршрут рядом с IPv4-маршрутами, поэтому IPv6 устройства уходит в туннель и там глохнет, а не идёт напрямую - разбор в разделе [Что видно сайту при делении трафика по адресу назначения](#split-detect-adv). Нужен IPv6 в туннеле - включите `--allow-ipv6-tunnel` (ниже). Нужна, наоборот, гарантия, что ничего не идёт мимо VPN, - отключите IPv6 на самом устройстве: серверный фильтр тут не поможет, потому что прямой IPv6-трафик до сервера просто не доходит.
...
- **Конфликт ULA-подсети.** Если `fddd:2c4:2c4:2c4::/64` уже используется в вашей сети, задайте другую ULA-подсеть через `IPV6_SUBNET=` до установки. Кроме `fddd:2c4:2c4:ffff::/64`: этот префикс занят под сток IPv6 режима маршрутизации 2, и установщик с таким значением остановится.
...
<a id="own-dns-adv"></a>
### Свой DNS-сервер для клиентов (Pi-hole, AdGuard Home, Blocky)

Блокировщик рекламы или свой резолвер можно поднять прямо на VPN-сервере и выдать клиентам в качестве DNS адрес сервера в туннеле. Установщик такой сервис не ставит, но и не мешает ему. Если что-то из списка ниже пропустить, схема не заработает, причём без единой ошибки:

1. **Сервис слушает адрес сервера в туннеле, взятый из конфигурации.** По умолчанию это `10.9.9.1`, но с `--subnet` адрес другой. Свой покажет `ip -4 addr show awg0` или строка `Address` в `/etc/amnezia/amneziawg/awg0.conf`. В командах ниже подставляйте его вместо `10.9.9.1`.
2. **Этот адрес выдан клиентам как DNS:** `sudo bash /root/awg/manage_amneziawg.sh modify <имя> DNS "10.9.9.1"` для каждого клиента. Чтобы новые клиенты сразу получали этот адрес, задайте `CLIENT_DNS` в `awgsetup_cfg.init`, см. [Изменение настроек по умолчанию](#change-defaults-adv).
3. **Адрес входит в `AllowedIPs` клиента.** Полный туннель (`0.0.0.0/0` в строке `AllowedIPs` клиентского конфига, режим по умолчанию с v5.34.0) его покрывает. Список режима 2 пускает частные сети мимо туннеля намеренно, а в своём списке режима 3 адреса нет, пока вы его не впишете. Это при включённой изоляции клиентов, она тоже по умолчанию. Тогда запрос идёт мимо туннеля и до сервера не доходит, а устройство может остаться без DNS или молча спросить DNS провайдера: блокировка выглядит настроенной, но не работает. Проще всего добавить адрес сервера в список клиента:

   ```bash
   # текущий список: всё после "AllowedIPs = ", вместе с IPv6-записями
   sudo sed -n 's/^AllowedIPs = //p' /root/awg/<имя>.conf
   sudo bash /root/awg/manage_amneziawg.sh modify <имя> AllowedIPs "<текущий список>, 10.9.9.1/32"
   ```

   Новым клиентам адрес нужно добавлять так же. Обычный `regen` такой список сохраняет, `regen --reset-routes` его сбросит.
4. **Порт 53 открыт только на `awg0`.** Если UFW включён (`sudo ufw status verbose` покажет `Default: deny (incoming)`), входящие закрыты, и без правила запросы из туннеля отбрасываются: `sudo ufw allow in on awg0 to any port 53`. Правило без `in on awg0` откроет резолвер всему интернету. Если сервис работает в Docker, UFW его опубликованные порты не фильтрует, поэтому порт нужно публиковать только на адрес туннеля: `-p 10.9.9.1:53:53/udp -p 10.9.9.1:53:53/tcp`.
5. **Файлы, скопированные с Windows, очищены от `CRLF`:** `sed -i 's/\r$//' <файл>`. Возврат каретки в конце строки списка может стать частью доменного имени, и такая строка перестанет блокировать.

После правки клиента его конфиг на устройстве нужно импортировать заново. Разбор этих граблей - от @Cueuler в [обсуждении #237](https://github.com/bivlked/amneziawg-installer/discussions/237).

<a id="change-defaults-adv"></a>
### Изменение настроек по умолчанию

**DNS новых клиентов** задаётся строкой `export CLIENT_DNS='...'` в `/root/awg/awgsetup_cfg.init`. Установщик пишет её туда пустой; впишите значение между кавычками, например `export CLIENT_DNS='10.9.9.1'`. На установках до этой версии строки нет, добавьте её в конец файла.

Значение - IP-адреса через запятую. Его получают клиенты, созданные после правки (`manage add`), и клиент, чей конфиг `regen` восстанавливает с нуля. Уже выданные конфиги не меняются: `regen` сохраняет их DNS, поменять его можно командой `modify <имя> DNS ...`. Невалидное значение молча не подменяется: `add` и установщик откажут и напишут, что не так, а `regen` существующих клиентов работает как прежде. Пустое значение означает `1.1.1.1, 1.0.0.1`. Переустановка с `--force` строку сохраняет.

**PersistentKeepalive** отдельной настройки не имеет: новые клиенты получают 33, у клиента значение меняется так:

```bash
sudo bash /root/awg/manage_amneziawg.sh modify <имя> PersistentKeepalive 25
```

Править саму библиотеку `awg_common.sh` бесполезно: установщик на шаге 5 и обновление скриптов перезаписывают её своей копией.
...
Опции:
  ...
  --protocol=2.0|3.1    Поколение протокола для НОВОЙ установки (умолч. 2.0). Принимается и
                        ...
                        3.1 эта версия установщика не выдаёт: откажет и назовёт причину
  ...
  --no-prebuilt         На ARM не ставить готовый пакет модуля из релиза arm-packages, а собрать модуль через DKMS
...
* **`modify <имя> <пар> <зн>`:** Изменить параметр клиента в `.conf` файле. Допустимые параметры: DNS, Endpoint, AllowedIPs, PersistentKeepalive. После изменения QR-код и vpn:// URI автоматически перегенерируются; прежние копии удаляются до правки, так что файл, который собрать не удалось, отсутствует, а не остаётся старым.
...
- `qr`/`vpnuri` - путь, если файл существовал на момент ответа. У `add` и `regen` QR и URI генерируются вне конфиг-блокировки: при параллельной операции файл может исчезнуть, гарантий свежести нет. У `modify` прежние копии удаляются до правки, а перевыпуск идёт под той же блокировкой: файл, который собрать не удалось, отсутствует, и поле равно `null`. Если параллельно с тем же клиентом ничего не делали, путь ведёт на копию из нового `.conf`.
- `results[].allowed_ips` (у `add`, записи со статусом `created`) - применённые маршруты клиента, прочитанные из только что созданного `.conf`. Может отличаться от аргумента `--allowed-ips`: полный туннель IPv4 дополняется IPv6-маршрутом (`::/0`, `2000::/3` или туннельной ULA, iOS-правило). `null` означает, что прочитать значение не удалось - причина идёт в журнал ошибкой, как у `qr`/`vpnuri`. Внешнему потребителю (боту) поле снимает проверочный вызов после создания (Issue #253).
...
- `check.protocol` - поколение протокола установки строкой (`"2.0"` или `"3.1"`), прочитанное из маркера `AWG_PROTOCOL` в `awgsetup_cfg.init`, а не из версии модуля: модуль третьей линии штатно работает с конфигурацией второй. Установка без маркера - это `"2.0"`. `check.protocol_error` в норме `null`; значение `"unreadable"` означает, что маркер испорчен (недопустимое значение, пустое поле, две строки маркера, файл не читается): тогда `protocol` равен `null`, `ok=false` и код возврата 1. Оба поля присутствуют в каждом ответе самой проверки. Если файла `awgsetup_cfg.init` нет или это не обычный файл, `check` завершается раньше проверки общим конвертом ошибки (`command`, `ok`, `error`, `rc`), и этих полей в нём нет.
...
<details>
  ...
  **О:** Используйте команду <code>modify</code> для каждого клиента: <code>sudo bash /root/awg/manage_amneziawg.sh modify &lt;имя&gt; DNS "8.8.8.8,1.0.0.1"</code>. Затем перегенерируйте конфиги: <code>sudo bash /root/awg/manage_amneziawg.sh regen</code>. DNS для новых клиентов задаётся строкой <code>CLIENT_DNS</code> в <code>awgsetup_cfg.init</code>, см. <a href="#change-defaults-adv">Изменение настроек по умолчанию</a>.
</details>
...
<details>
  ...
  Если рукопожатие так и не собирается, снижать junk дальше бесполезно: на замеренном маршруте дело было в <code>I1</code>, см. <a href="#no-hs-mobile-adv">разбор</a>.
  <br><br>
  ...
  <tr><td>МТС (Москва)</td><td>случайный I1=&lt;r 96&gt; и длиннее не проходит, &lt;r 64&gt; и I1 формы DNS проходят; Jc/Jmin/Jmax не влияют (замер, сентябрь 2026)</td><td>I1 формы DNS (на новых установках с v5.33.0 его пишет установщик) или &lt;r 64&gt;, см. <a href="#no-hs-mobile-adv">разбор</a></td><td>✅</td></tr>
...
> Рядом лежит `<имя>.png` - QR из `.conf` для клиентов AmneziaWG, которые умеют импорт по QR-коду; AmneziaWG для Windows и `awg-quick` берут тот же `.conf` файлом, а обычный клиент WireGuard его не примет. Это разные форматы с разными получателями: приложение Amnezia VPN сканирует `.vpnuri.png`, клиенты AmneziaWG с импортом по QR - `<имя>.png`. Не путайте.
...
4. Готовый пакет можно не брать вовсе: с флагом `--no-prebuilt` модуль сразу собирается через DKMS. Готовые пакеты собирает CI проекта, а установщик сверяет их только с файлом SHA256 из того же релиза; подписи у них нет. Учтите, что меняется и сам модуль: готовые пакеты собраны из закреплённого исходника AmneziaWG 2.0, а на ядрах 6.7 и новее DKMS ставит `amneziawg-dkms` из PPA, то есть тот же модуль третьей линии, что и на сервере x86. Клиентские профили в обоих случаях второго поколения.
...
Ищите в логе установки (<code>/root/awg/install_amneziawg.log</code>) строку <code>Предсобранный пакет установлен</code> (английский установщик пишет <code>Prebuilt installed</code>). Если вместо неё есть <code>используется DKMS</code>, модуль собран из исходников.
</details>
...
* **Ubuntu вне `noble`, `jammy` и `focal` (например, 25.10 и 26.04):** установщик проверяет, есть ли в PPA пакеты под кодовое имя выпуска, и если нет (ответ 404), переключает codename PPA на `noble` (с v5.13.0). Debian отображается на ближайший выпуск Ubuntu всегда: 12 -> `focal`, 13 -> `noble`. Модуль в обоих случаях собирается через DKMS под ваше ядро, поэтому первая установка идёт дольше.
