...
| AmneziaWG в Google Play (лёгкий) | **вывод по дате: вторая линия** | номер версии Play не публикует | листинг обновлён **12 июн 2026** | Google Play |
| AmneziaWG для Windows (отдельный клиент) | третья линия | 3.1.0 | 21 авг 2026 | страница релизов проекта |
| WG Tunnel (сторонний, Android) | умеет AmneziaWG; поколение нами не проверялось | 5.6.0 | 27 авг 2026 | Play, F-Droid, IzzyOnDroid, APK |
...
на странице приложения, поэтому поколение выводится из дат: листинг обновлён 12 июня 2026,
релиз того же приложения от 12 июня относится ко второй линии, а сборка третьей линии вышла
14 августа 2026, то есть уже после последнего обновления листинга. Прочитать номер версии прямо
нельзя, и мы не утверждаем, что читали.

⚠️ **Та же строгость к строке WG Tunnel.** Это сторонний проект, его релиз 5.6.0 от 27 августа
2026 существует, но поколение протокола в нём поведением не проверялось и цифра не называется.
До проверки он стоит в таблице как «умеет AmneziaWG», не более.

> ⚠️ Поколение у сторонних проектов указано по документации и релиз-нотам их авторов: их код мы
> читали, но поведением не проверяли ни один. Строка, проверенная нами, помечена явно.
