...
- `check.protocol` - поколение протокола установки строкой (`"2.0"` или `"3.1"`), прочитанное из маркера `AWG_PROTOCOL` в `awgsetup_cfg.init`, а не из версии модуля: модуль третьей линии штатно работает с конфигурацией второй. Установка без маркера, в том числе без самого файла `awgsetup_cfg.init`, - это `"2.0"`. `check.protocol_error` в норме `null`; значение `"unreadable"` означает, что маркер испорчен (недопустимое значение, пустое поле, две строки маркера, существующий файл не читается, на его месте каталог, FIFO, устройство или битая символьная ссылка): тогда `protocol` равен `null`, `ok=false` и код возврата 1. Оба поля присутствуют всегда.
