...
**Причина и что делать.** По умолчанию установщик записывает маскирующий пакет как `I1 = <r N>`, то есть N случайных байт. Часть мобильных сетей такой пакет отбрасывает. Проверенные способы, в порядке предпочтения:

1. **Заменить `I1` на пакет осмысленной формы.** Строка вида `<r 2><b 0x8580...>` изображает ответ DNS: два случайных байта - идентификатор транзакции, дальше заголовок и запись. Готовую строку под QUIC можно получить генератором из раздела [Блокировка по AS и обход через I1/CPS](#as-blocking-adv); он же годится и здесь, потому что решает не размер пакета, а его форма.
