...
| **Блокируется в** | Китай, Россия, Иран, ОАЭ, Туркменистан | Блокировок по сигнатуре протокола не встречал (наблюдение на август 2026, не гарантия). Отдельные сети режут UDP-порты - помогает смена порта и `--mobile` |
...
| **Потеря скорости** | Базовая | Небольшая: модуль ядра, шифрование и маскировка без выхода в userspace |

> Про скорость: абсолютные цифры упираются в процессор сервера, потому что шифрование и маскировка считаются на нём же. На одном общем ядре узким местом будет он. Диагностика: [FAQ в ADVANCED](ADVANCED.md#faq-advanced-adv).
...
2.  **Скачайте скрипт:**
    ```bash
    wget -O install_amneziawg.sh https://raw.githubusercontent.com/bivlked/amneziawg-installer/v5.28.1/install_amneziawg.sh
    # или: curl -fLo install_amneziawg.sh https://raw.githubusercontent.com/bivlked/amneziawg-installer/v5.28.1/install_amneziawg.sh
    ```
