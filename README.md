...
Установщик настраивает обфускацию AmneziaWG 2.0 под мобильные сети с DPI: флаг `--mobile` включает мобильный пресет и порт 443/udp одной командой. Если VPN нестабилен через мобильный интернет - переустановите с `--mobile`. Ниже - рабочие конфигурации по отчётам пользователей из issues и discussions (не гарантия: блокировки и параметры операторов меняются со временем):
...
> **🤖 Для скриптов и ботов:** с v5.21.0 команды управления (`add`, `remove`, `regen`, `modify`, `backup`, `restore`, `check`, `restart`, `repair-module`) принимают `--json` и печатают ровно один JSON-документ при любом исходе; `list --json` и `stats --json` по-прежнему отдают простые массивы. Контракт полей и строгий режим подтверждений `AWG_STRICT_CONFIRM=1` - в [ADVANCED.md → JSON-интерфейс](ADVANCED.md#json-api-adv). Готовый Telegram-бот на этом интерфейсе - [awgram](#ekosistema).
...
<details>
  ...
  <br><br>
  С v5.20.1 manage сверяет совместимость версии awg_common со своей: несовместимая пара (например, обновили только одну половину до нового минорного выпуска) не сбоит загадочными ошибками, а останавливается с готовыми командами для обновления обеих.
</details>
...
<details>
  ...
  <b>О:</b> Да. Команды управления принимают флаг <code>--json</code> и печатают ровно один JSON-документ при любом исходе - удобно парсить из скриптов, CI и ботов (с v5.21.0; <code>list</code>/<code>stats</code> отдают JSON-массивы давно). Формат стабильный: новые поля могут добавляться, существующие не переименовываются и не меняют тип. Для неинтерактивных запусков есть <code>--yes</code> и строгий режим <code>AWG_STRICT_CONFIRM=1</code>: без явного <code>--yes</code> деструктивная команда откажется, а не согласится молча. Детали - в <a href="ADVANCED.md#json-api-adv">ADVANCED.md → JSON-интерфейс</a>. Готовый Telegram-бот поверх этого интерфейса - <a href="https://github.com/ekuraev/awgram">awgram</a>.
</details>
