...
<p align="center">
  ...
  <a href="https://github.com/bivlked/amneziawg-installer/releases"><img src="https://img.shields.io/badge/Installer_Version-5.36.2-blue" alt="Version"></a>
...
**Факты на 2026-09-24.** Установщик 5.36.2. Сроки поддержки ОС проверены по данным вендоров на 2026-08-29.
...
* **Сервер оптимизируется под железо.** Скрипт смотрит на RAM и сетевую карту и настраивает sysctl-буферы и размер swap, включает BBR - вытягивает из тарифа максимум. Официальное приложение разворачивает контейнеры и сам сервер не оптимизирует и не настраивает.
...
* Hardware-aware оптимизация: swap и сетевые буферы на основе характеристик сервера
...
5.  **Начальная настройка:** Скрипт интерактивно запросит:
    ...
    * **Отключение IPv6:** рекомендуется (`Y`). Речь об IPv6 на самом сервере; попадёт ли в туннель IPv6 вашего устройства, решает режим маршрутизации, см. [IPv6 устройства, `::/0` и локальная сеть](ADVANCED.md#client-ipv6-adv).
...
| [amneziawg_dns_adblock](https://github.com/Cueuler/amneziawg_dns_adblock) | Сервер | Блокировка рекламы и трекеров в DNS для клиентов VPN от @Cueuler: Blocky на адресе туннеля, ставится поверх готовой установки, нужен полный туннель (`--route-all`). Файла лицензии нет, автор разрешил свободное использование в [обсуждении #237](https://github.com/bivlked/amneziawg-installer/discussions/237#discussioncomment-18605625) |
