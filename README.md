...
| Ubuntu 26.04 | ✅ можно брать | до 2031-05-29 | Установщик проверяет, есть ли в PPA пакеты под кодовое имя выпуска, и если нет, сам переключается на `noble` (с v5.13.0) |
...
| Ubuntu 25.10 (questing) | ⚠️ для миграции | закончилась 2026-07-01, продления нет | Работает: пакетов под `questing` в PPA нет, установщик сам переключается на `noble` (с v5.13.0). Обновлений безопасности не выходит вообще никаких - для нового сервера не берите |
