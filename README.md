...
7.  **Продолжение:** После каждой перезагрузки **снова запустите скрипт** той же командой:
    ...
    sudo bash ./install_amneziawg.sh
    ```
    Скрипт продолжит с нужного шага, прежние вопросы не повторяются. Один новый вопрос будет в третьем запуске: «Включить UFW? [y/N]». Ответьте `y`: по Enter фаервол не включается, и сервер остаётся без него. С `--yes` UFW включается без вопроса.
...
**Ключ сверяйте целиком.** Публичный ключ проекта: `RWQXfpABHIpZPttqrwYrQNHRTk/iLIz4cVh9KkRwAElHP+CoW/NPEysN`. Он же лежит в `KEYS.txt` в корне репозитория и в каждом релизе. `3E598A1C01907E17` - идентификатор ключа, а не отпечаток: чужой ключ может нести тот же идентификатор, поэтому сверять по нему нельзя. Сохраните ключ у себя и в следующий раз берите его из своей копии.

**Что покрывает подпись.** Вы проверяете сам установщик. Скрипты, которые он качает потом (`awg_common.sh`, `manage_amneziawg.sh`), он сверяет с SHA256-хешами, зашитыми в него, так что подписанный установщик покрывает и их. Не покрыты подписью:
...
<details>
  ...
  <br><br>
  Оговорка одна: если передать <code>--mobile</code>, <code>--preset</code>, <code>--jc</code>, <code>--jmin</code> или <code>--jmax</code>, весь набор <code>Jc</code>/<code>S</code>/<code>H</code>/<code>I1</code> сгенерируется заново, а <code>--port</code> (и <code>--mobile</code>, который ставит 443) меняет порт в <code>Endpoint</code>. В обоих случаях старые конфиги перестанут подключаться, их придётся перевыпустить через <code>regen</code> и раздать снова. При обычном обновлении эти флаги не указывайте.
  <br><br>
