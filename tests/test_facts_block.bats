...
@test "facts: битые маркеры во ВТОРОМ файле не дают переписать первый" {
    # Пара файлов правится не атомарно, поэтому разбор ОБОИХ обязан пройти до
    # первой записи. Прежняя форма шла циклом "проверил-записал": русский файл
    # уже переписан, на английском обнаружились битые маркеры -> отказ, и
    # репозиторий в половинчатом состоянии. Нашло ревью бота на PR #241.
    _sub README.md "Установщик $VER" "Установщик 9.99.9"
    _sub README.en.md "<!-- facts:end -->" ""
    before="$(_hashall README.md)"
    run _write
    [ "$status" -eq 1 ]
    [[ "$output" == *"README.en.md"* ]]
...
@test "facts: комментарий В КОНЦЕ строки сторожа не ослепляет" {
    ...
    printf '\n    echo "HeaderProtectionKey = $k" >> "$conf"  # 3.1, см. MyAI-9zrc\n' \
        >> "$TMP/manage_amneziawg.sh"
    run _check
    [ "$status" -eq 1 ]
    [[ "$output" == *"manage_amneziawg.sh"* ]]
}
