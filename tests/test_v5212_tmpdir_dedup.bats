#!/usr/bin/env bats
# v5.21.2:
#   - tmpdir-cleanup: manage_mktempdir_var registers the temp dir in the PARENT array, so
...
# --- tmpdir-cleanup: registration survives in the parent shell ---
...
@test "v5.21.2 (tmpdir-cleanup): manage_mktempdir_var writes the caller local and registers in the parent (RU)" {
    run _assert_registers "$BATS_TEST_DIRNAME/../manage_amneziawg.sh"
    [ "$status" -eq 0 ] || { echo "$output"; return 1; }
}

@test "v5.21.2 (tmpdir-cleanup): manage_mktempdir_var writes the caller local and registers in the parent (EN)" {
...
@test "v5.21.2 (tmpdir-cleanup): manage_mktempdir_var fails cleanly when mktemp fails (RU)" {
    run _assert_fails_clean "$BATS_TEST_DIRNAME/../manage_amneziawg.sh"
    [ "$status" -eq 0 ] || { echo "$output"; return 1; }
}

@test "v5.21.2 (tmpdir-cleanup): manage_mktempdir_var fails cleanly when mktemp fails (EN)" {
...
# registered dir. This closes the loop on the registration fix - the leak was that the handler
...
@test "v5.21.2 (tmpdir-cleanup): _manage_cleanup removes a registered temp dir (RU)" {
    run _assert_cleanup_removes "$BATS_TEST_DIRNAME/../manage_amneziawg.sh"
    [ "$status" -eq 0 ] || { echo "$output"; return 1; }
}

@test "v5.21.2 (tmpdir-cleanup): _manage_cleanup removes a registered temp dir (EN)" {
    run _assert_cleanup_removes "$BATS_TEST_DIRNAME/../manage_amneziawg_en.sh"
    [ "$status" -eq 0 ] || { echo "$output"; return 1; }
}

@test "v5.21.2 (tmpdir-cleanup): the old subshell-registering manage_mktempdir is gone (RU+EN)" {
