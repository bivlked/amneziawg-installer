#!/usr/bin/env bats
# D#38 (@humowns) - regen and the vpn:// URI builder silently collapsed the
# comma-space separator in AllowedIPs and DNS.
#
# The installer writes those lists as "a, b, c". Reading them back went through
# `tr -d '[:space:]'` (regen) and `tr -d ' \r'` (URI builder), which stripped the
# spaces together with CR, and regen then wrote the collapsed list into .conf.
# The damage is invisible to the eye and survives every later regen.
#
...
@test "D#38: both manage variants normalize AllowedIPs and DNS before writing" {
    for f in manage_amneziawg.sh manage_amneziawg_en.sh; do
        run grep -c 'AllowedIPs|DNS) value=$(awg_normalize_csv "$value") ;;' "$BATS_TEST_DIRNAME/../$f"
        [ "$output" = "1" ]
    done
}

@test "D#38: modify normalizes before escaping, not after" {
    for f in manage_amneziawg.sh manage_amneziawg_en.sh; do
        local norm_line esc_line
        norm_line=$(grep -n 'AllowedIPs|DNS) value=$(awg_normalize_csv' "$BATS_TEST_DIRNAME/../$f" | cut -d: -f1)
        esc_line=$(grep -n 'escaped_value=$(escape_sed "$value")' "$BATS_TEST_DIRNAME/../$f" | cut -d: -f1)
        [ -n "$norm_line" ]
        [ -n "$esc_line" ]
        [ "$norm_line" -lt "$esc_line" ]
    done
}
