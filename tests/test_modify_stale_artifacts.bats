...
# refused by validation keeps the files. The JSON envelope reports qr and vpnuri
# the way add does: a path when the file exists, null when it does not.
#
...
teardown() {
    # Stubs (rm, qrencode) live in $MGMT_DIR/bin: drop them from PATH before
    # the directory goes, or bats itself calls a removed rm after the test.
...
# qrencode stubs: one that always fails, one that writes a marker to its -o
# target, and a probe that records whether PROBE_TARGET exists when it runs.
stub_qrencode_fail() {
...
successful_rebuild_reports_paths() {
    ...
    [ "$status" -eq 0 ]
    grep -qxF 'fresh' "$A/alice.png"
    grep -qxF 'fresh' "$A/alice.vpnuri.png"
    grep -q '^vpn://' "$A/alice.vpnuri"
    one_json_line
...
unremovable_file_refuses_before_the_edit() {
    ...
    one_json_line
    printf '%s' "$output" | jq -e '.command == "modify" and .ok == false and .rc == 1' >/dev/null
    [[ "$stderr" == *"alice.png"* ]]
}
...
@test "modify RU: a successful rebuild replaces the old files and reports their paths" {
    successful_rebuild_reports_paths
}
...
@test "modify RU: a value refused by validation keeps the files" {
    refused_value_keeps_the_files
}
...
@test "modify EN: a successful rebuild replaces the old files and reports their paths" {
    use_en
    successful_rebuild_reports_paths
}
...
@test "modify EN: a value refused by validation keeps the files" {
    use_en
    refused_value_keeps_the_files
}
