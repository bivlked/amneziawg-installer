...
@test "every awg show call site is bounded by a timeout" {
    ...
    # принимал только `timeout N` и краснел на более строгом вызове; формы
    # `-s KILL 5` и `--kill-after=1 5` он тоже отвергал, хотя они ограничены.
    # 🔴 Что образец НЕ проверяет: чьё это число перед командой. В форме
    # `timeout -k 1 awg show` число принадлежит флагу, длительности нет, и образец
    # такую строку ПРИНИМАЕТ. Это не дыра для неограниченного вызова - coreutils
    # сам отвергает её («invalid time interval»), - но обещать больше, чем
    # проверяется, здесь нельзя.
    for f in "$MANAGE" "$MANAGE_EN"              "${BATS_TEST_DIRNAME}/../install_amneziawg.sh"              "${BATS_TEST_DIRNAME}/../install_amneziawg_en.sh"; do
        run bash -c 'strip() { sed "s/\"[^\"]*\"//g" "$1" | grep -vE "^[[:space:]]*#"; }
            ...
            bounded=$(strip "$1" | grep -E "(^|[^_[:alnum:]-])awg show" | grep -cE "timeout( +(-[A-Za-z0-9]+|--[a-z-]+(=[A-Za-z0-9]+)?)( +[A-Za-z0-9]+)?)* +[0-9]+[smhd]? +awg show") || true
