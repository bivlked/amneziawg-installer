...
Установка перезагрузит сервер дважды (с `--yes` без вопроса). После каждой перезагрузки запустите ту же команду снова, закончится она строкой об успешном завершении.
...
`::/0` оставьте как есть: при `Table = off` маршрутов он не добавляет.
