...
# Скрипт ждёт блокировку до 5 минут, а systemd по умолчанию даёт oneshot-юниту только 90 секунд
# и потом убивает его. Без этой строки юнит в редком случае ожидания молча не применил бы деление.
# 600 - это 300 секунд ожидания блокировки плюс запас на саму работу (закачка списка и ~8600
# вызовов ipset на слабой машине). Ставить 360 нельзя: на работу осталось бы всего минута.
TimeoutStartSec=600
