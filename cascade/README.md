...
репозитории - запасной вариант: если ipdeny.com недоступен, скрипт берёт файл отсюда
через `raw.githubusercontent.com`, и только если недоступно и это - уже лежащую на
сервере копию. Так каскад поднимается с рабочим списком RU-сетей и на свежем сервере.
...
a fallback: if ipdeny.com is unreachable, the script fetches this file over
`raw.githubusercontent.com`, and only if that fails too does it keep the copy already on the
server. That way the cascade comes up with a working RU network list even on a fresh server.
