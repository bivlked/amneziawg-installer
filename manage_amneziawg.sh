...
list_clients() {
    ...
    while IFS= read -r name; do
        ...
        # файлы на сервере. Прежде чтение было только для таблицы ради экономии
        # одного открытия файла на клиента; экономия не стоит дыры в контракте.
        # Принимаем только числовой timestamp: повреждённый expiry-файл дал бы
        # ошибку bash-арифметики из format_remaining прямо в таблице.
        # Числом считаю ТОЛЬКО каноническую десятичную запись. Ведущий ноль
        # отвергаю сознательно, и это не педантизм: во-первых, JSON запрещает
        # такие числа, и строгий разборщик у потребителя (Python, .NET, Go)
        # уронил бы ВЕСЬ документ из-за одного клиента; во-вторых, bash считает
        # такое значение восьмеричным, поэтому крон на нём падает с 'value too
        # great for base' и клиента не удаляет - то есть срок и так уже не
        # работает. Честный ответ на такую метку - 'не читается', а не число.
        # Оба ключа присутствуют ВСЕГДА, «неприменимо» выражается значением
        # null. Это здешняя конвенция, а не вкус: в add поля qr, vpnuri и
        # expires_at тоже всегда на месте, а отсутствие файла даёт null. Ключ,
        # появляющийся только иногда, делает форму записи зависящей от данных,
        # и строгий потребитель со сверкой набора полей на этом спотыкается -
        # ровно так спотыкается наш собственный контрактный тест.
        ...
        if [[ "$exp_ts" =~ ^(0|[1-9][0-9]*)$ ]]; then
            ...
            # Метка есть, но не разбирается. В JSON это НЕ просто null: null
            # означает «клиент бессрочный по замыслу», а тут срок кто-то ЗАДАЛ,
            # и он молча не работает - check_expired_clients такую метку
            # пропускает с предупреждением в журнал и клиента не удаляет
            # НИКОГДА. Это сломанное состояние, которое надо чинить, поэтому
            # рядом идёт признак expires_at_error, оставляющий его видимым.
            _jexp_err='"unreadable"'
            [[ "$JSON_OUTPUT" -ne 1 ]] && exp_str=" [expiry повреждён]"
        fi
