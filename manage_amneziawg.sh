...
# Порт из awgsetup_cfg.init числом не является: файл правят руками, и там
# оказывается что угодно. Ломались два места - JSON (значение идёт без кавычек,
# и "number":abc не разбирается) и [[ -eq ]], где bash считает арифметику и
# выполняет подстановку команд из строки вида a[$(...)].
# Функция, а не пара строк по месту: так её исполняет тест, а не копия логики.
_sanitize_port() {
    local p="${1:-}"
    # {1,5} отсекает переполнение 64-битной арифметики, 10# - восьмеричную
    # трактовку значений вида 0080.
...
check_server() {
    ...
    if [[ "$port" -eq 0 ]]; then
        if [[ -n "${AWG_PORT:-}" ]]; then
            ...
            # такая же поломка, как незапущенный сервис. Значение обрезаю: там
            # может оказаться сколь угодно длинная строка.
            log_error " - Порт в конфиге некорректный: '${AWG_PORT:0:32}'."
            ok=0
        else
...
diagnose_server() {
    ...
    safe_load_config "$CONFIG_FILE" 2>/dev/null
    # Тот же непроверенный порт раньше попадал прямо в regex: значение вроде
    # '.*' или '[' давало ложное совпадение или сломанный шаблон.
    local awg_port
    ...
    if command -v ufw &>/dev/null; then
        ...
        if [[ "$awg_port" -eq 0 ]]; then
            _diag_line WARN "Порт в конфиге некорректный, проверка правила UFW пропущена"
            warn=$((warn+1))
