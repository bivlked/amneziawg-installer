...
modify_client() {
    ...
    case "$param" in
        DNS)
            # Структурная проверка списка DNS: только IPv4/IPv6 через запятую, без
            # FQDN. Живёт в библиотечном awg_validate_dns_list и разделяется с
            # CLIENT_DNS из awgsetup_cfg.init: две инлайн-копии тихо разъезжаются, а
            # прежний цикл здесь раскрывал шаблоны в текущем каталоге.
            command -v awg_validate_dns_list >/dev/null 2>&1 || {
                log_error "awg_common.sh устарела: нет awg_validate_dns_list. Обнови обе половины под одну версию."
                return 1
            }
    ...
    case "$param" in
        AllowedIPs|DNS)
            ...
            # него же снимают IPv6-маршрут те, кому нужен работающий IPv6. Но
            # если заданный список это полный туннель БЕЗ IPv6-маршрута (::/0 или
            # 2000::/3), клиент теряет маршрут, который ему дают add и regen, и
            # его IPv6 снова идёт мимо туннеля. Публичные рецепты вида
...
list_clients() {
    ...
    while IFS= read -r name; do
        ...
        #
        # Числом считается ТОЛЬКО каноническая десятичная запись не длиннее 15
    ...
    add)
        ...
        for _cname in "${ARGS[@]}"; do
            ...
            if generate_client "$_cname"; then
                ...
                # IPv4 дополняется IPv6-маршрутом (iOS-правило). Боту это снимает
