...
# и там оказывается что угодно. Ломались два места - JSON (значение идёт без
# кавычек, и "number":abc не разбирается) и [[ -eq ]], где bash считает
# арифметику и выполняет подстановку команд из строки вида a[$(...)].
