...
# Номер порта из awgsetup_cfg.init - к нему нельзя относиться как к числу:
# файл правят руками, и после неудачной правки там оказывается что угодно.
# Ломались сразу два места. В JSON значение подставляется без кавычек, и
# "number":abc уже не разбирается парсером, хотя контракт обещает ровно один
# валидный документ. А в [[ "$port" -eq 0 ]] bash считает арифметику, и
# значение вида a[$(...)] там ВЫПОЛНЯЕТСЯ. Файл лежит под root с правами 600,
# так что чужой это не подсунет, но полагаться на права там, где хватает
# одной проверки, незачем.
# Отдельная функция, а не пара строк по месту: так её исполняет тест, а не
# его собственная копия логики.
_sanitize_port() {
    local p="${1:-}"
