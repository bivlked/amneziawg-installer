    ...
    while IFS= read -r name; do
        ...
        if [[ ! "$name" =~ ^[a-zA-Z0-9_-]+$ ]]; then
            ...
            if [[ "$_exp_rc" -ne 0 ]]; then
                # Файл есть, а прочитать не вышло: права, ошибка ввода-вывода,
                # обрыв чтения. Код возврата тут единственный источник правды -
                # вывод в этом случае пуст и неотличим от пустого файла.
