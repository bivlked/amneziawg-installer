...
- **What the installer changes and what `--uninstall` leaves.** README lists the changes the way the code makes them now. README and both install guides say what removal takes away and what stays on the server: dependency packages, swap, the module package on ARM, and the `/root/awg_uninstall_backup_*.tar.gz` archive with configs and private keys, which you have to delete yourself.
...
- **Corrections in ADVANCED and README.** Containers: the installer stops inside them, and the LXC path is `amneziawg-go`. Moving to a new server: check the network interface, `ListenPort` and `AWG_ENDPOINT`. `regen` takes the address from `AWG_ENDPOINT`. The "Amnezia list" mode is no longer called split routing. The switch to `noble` is described as conditional. The English README gives `I1 = <r 48>` for Tele2 (Krasnoyarsk).
- **Issue templates.** The issue and Q&A templates ask for the client app, the network and the routing mode, show how to get the version and link the English FAQ. The hoster affiliate link is marked as one.
