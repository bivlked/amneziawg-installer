...
- **An install on a kernel older than 6.7 (Debian 12, for example) no longer stops with "Failed to hold amneziawg-dkms" while the hold is actually in place** (issue #285). After `apt-mark hold` the installer checked the result against the `apt-mark showhold` list piped into `grep`, and on some systems it refused with the hold in place, including the `hold ok not-installed` state. The check now looks where apt itself takes the hold from, the package selection in dpkg, and reads the `apt-mark showhold` list as a second source without a pipe. If neither shows the hold, the install still stops: without it `amneziawg-tools` would pull the AmneziaWG 3.0 module from the PPA, bypassing the chosen path. The same check runs before the tools are installed on ARM with a prebuilt module.
