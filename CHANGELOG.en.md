...
- **The header protection key no longer reaches the `manage show` output or the `diagnose` report when `WG_COLOR_MODE=always` is set.** With that variable `awg show` colours its labels even when the output is not a terminal, and the secrets filter did not recognise the `header protection key` line. The scripts now run the tools with `WG_COLOR_MODE=never`, which also brings back the `jc`, `jmin` and `jmax` parsing in `diagnose` under that variable.
