...
- **`diagnose` warns about `Jmin` greater than `Jmax` on the live interface.** Such a pair is invalid, and the kernel module does not check it and writes past a buffer end with it ([amneziawg-linux-kernel-module#225](https://github.com/amnezia-vpn/amneziawg-linux-kernel-module/issues/225)). The installer never produces this pair; it only appears after a hand edit of `awg0.conf`. A missing `Jmax` with `Jmin` set counts as the same error.
