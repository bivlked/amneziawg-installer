...
- **Script updates with signature verification.** The update block in README and ADVANCED downloads `manage_amneziawg.sh` and `awg_common.sh` into a temporary folder, verifies each file with `minisign -P` and replaces the working copies in `/root/awg/` only after a successful check. The integrity line in README matches the download from the latest release.
