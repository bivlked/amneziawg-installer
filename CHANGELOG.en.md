...
- **The DNS for new clients is set in `awgsetup_cfg.init`.** A line `export CLIENT_DNS='10.9.9.1'` (IPs separated by commas) gives that DNS to clients created after the edit and to a client whose config `regen` rebuilds from scratch. Configs already handed out keep their DNS. An invalid value is not silently replaced: creating a client and the installer refuse with a clear message, while `regen` of clients already handed out works as before. An empty value means `1.1.1.1, 1.0.0.1`. A line the parser does not recognise (spaces around `=`, an indent, lower case) is named in a warning by the installer and `manage` instead of being skipped silently. Before this the only way was to edit the library, and the installer and script updates overwrote that edit. Idea from [PR #294](https://github.com/bivlked/amneziawg-installer/pull/294) by @rootcreep.
- **The `--no-prebuilt` flag.** On ARM the installer skips the prebuilt module package from the `arm-packages` release and builds the module with DKMS. The prebuilt packages are built by this project's CI and checked only against a SHA256 from the same release, with no signature; the flag is for those who care. On kernels 6.7 and newer it is also a different module: instead of the pinned 2.0 one, `amneziawg-dkms` from the PPA is installed, as on x86. If a prebuilt package is already there from an earlier run, the installer stops and says how to remove it, so no second module is built next to it. A `NO_PREBUILT` other than 0 or 1 stops the install too. The choice is saved in `awgsetup_cfg.init` and survives the install's reboots.
...
- **`--ssh-port` without a valid port stops the install.** A value such as `2222/tcp` or `222222` used to become 22 silently, and with `--yes` UFW came up with a rule for port 22 only, cutting off SSH on the real port. The installer now stops already at step 0 with a clear message and leaves UFW alone. An overlong number is refused too instead of wrapping into a small port number. An explicit list is taken only as a whole: `--ssh-port=22,500022` (one digit too many in 50022) stops the install instead of leaving port 22 alone. An empty `--ssh-port=` and an empty list element (`22,`, which is what an unset variable looks like) are refused too, and the value is not expanded as a file name pattern. Without the flag the SSH port is detected automatically, as before.
- **The `restart` apply mode works everywhere it is promised.** `--apply-mode=restart` and `AWG_APPLY_MODE=restart` in the environment are no longer overridden by the value in `awgsetup_cfg.init` during `add`, and a mode saved in the init file is now honoured by `remove` and by the cron removal of expired clients as well. The order is: flag or environment, then the init file, then `syncconf`. An unknown value (such as `Restart`) still gives `syncconf`, now with a warning.
- **Step 7 no longer mistakes a working service for a broken one on a server with many clients.** The interface, port and obfuscation checks read their output through `| grep -q` under `pipefail`, and on long output the interface or port check could fail by mistake and the parameter check could give a false warning. The output is now read in full first.
- **`manage check` no longer says "port is not listening" about a listening port.** Same cause: `| grep -q` under `pipefail` on a long socket list.
- **`manage diagnose` no longer hangs on an `I1` with a space inside a tag length**, such as `<r 1 0>`. The tag parser lost its position and went round in circles.
- **An empty `--expires=` in `add` is now an error.** The client used to be created permanent without a word while JSON reported success; this happens when a bot passes an empty expiry variable. Without the flag the client is permanent, as before.
- **The installer checks the write of `awgsetup_cfg.init`.** On a full disk an empty or truncated file replaced the working one, the next run generated new obfuscation parameters, and the clients already handed out stopped connecting. A failed write now stops the installer before the replace, and the working file stays as it was.
- **An expiry stamp no longer attaches to a same-named permanent client.** After `restore` or a new `add` with the same name, a client could inherit the deadline of the previous one and be deleted by cron at that deadline. `add` and `restore` now drop such a stamp; if it cannot be removed, `add` refuses that client and `restore` rolls back. Stamps from the archive are restored as before, and `restore` now checks that they actually landed, rolling back otherwise. An archive without an `expiry/` directory had no deadlines in it, so clients restored from it lose their stamps. If `restore` rolls back, the rollback also takes away stamps that came from the archive, so a client that was permanent before `restore` stays permanent.
- **`IPV6_SUBNET` can no longer land in the IPv6 sink prefix.** The routing mode 2 sink lives in `fddd:2c4:2c4:ffff::/64` and is recognised by that prefix. With the tunnel subnet set inside it, a client's real IPv6 address was taken for a sink: `regen` and `modify` could drop it, and `vpn://` and `list` did not show it. The installer with the IPv6 tunnel now stops on such a value already at step 0, and creating a client refuses with a clear message. On a server that already has clients the installer only warns: the subnet cannot change under live clients, and the clean path is `--uninstall` and an install with another ULA subnet.
- **The English `manage` warning about an outdated library is no longer cut off.** The ADVANCED section name sat in double quotes inside a double-quoted string, so the message stopped at "(section How". The Russian version was already correct.
...
- **Changing the MTU now mentions the MSS.** The FAQ "How do I change the MTU?" and the "reduce MTU" troubleshooting step say that `--set-mss` in `PostUp` and `PostDown` must change along with the MTU (MTU minus 40 for IPv4, minus 60 for IPv6), with the tunnel stopped before the edit. Without it, large packets stop passing through the tunnel after the MTU is lowered ([discussion #38](https://github.com/bivlked/amneziawg-installer/discussions/38)). The MSS section now says that `--force` recomputes the MSS from the new MTU only without the `--preset`, `--jc`, `--jmin`, `--jmax` flags: with them the manual MTU edit is rolled back.
- **Your own DNS server for clients.** ADVANCED gains a section on giving clients an ad blocker or your own resolver running on the VPN server (Pi-hole, AdGuard Home, Blocky), and on what breaks that setup without a single error: an example address instead of the configured one with `--subnet`, a DNS address never given to clients, the DNS address outside the client's `AllowedIPs` with the list-based routing modes, port 53 closed on `awg0` (and open to the whole internet when the resolver runs in Docker), and `CRLF` in files copied over from Windows ([discussion #237](https://github.com/bivlked/amneziawg-installer/discussions/237)).
- **The README first screen describes the install more accurately.** The installer creates two clients, `my_phone` and `my_laptop`, and saves their configs, QR codes and `vpn://` links in `/root/awg/` rather than printing them; it offers to turn on the UFW firewall. The Russian subtitle now names the project, and a link to the English README sits next to it.
- **The right QR code for the Amnezia VPN app.** It is `name.vpnuri.png`, the QR of the `vpn://` link; `name.png` is the QR of the `.conf` itself, for AmneziaWG clients. The README and the English install guide used to point to `name.png`.
- **Two installer questions that are easy to miss are described.** The package cleanup question and "Enable UFW? [y/N]" in the third run: pressing Enter leaves the firewall off.
- **The full list of flags after which configs must be reissued.** Besides `--preset`, `--jc`, `--jmin` and `--jmax` these are `--mobile` and `--port`: they change the obfuscation parameters or the port. `--endpoint` is covered separately: only reissued configs get the new address. The mobile advice no longer suggests simply reinstalling a running server with `--mobile`.
- **ARM and kernel upgrades.** The FAQ answer and the guide say that on ARM with a prebuilt module there is no DKMS, and the installer has to be run again after a kernel change.
- **Signature verification compares the whole key.** The command uses `minisign -P` with the full public key, and `3E598A1C01907E17` is called a key ID rather than a fingerprint: another key can carry the same ID. The section explains what the signature covers and what it does not give you. The same in SECURITY and SIGNING_DESIGN.
- **Five new questions in the README FAQ**: mobile data, the yellow config in the Amnezia app, the local network and IPv6, Synology, and "stopped connecting after an update".
- **Script updates with signature verification.** The update block in README and ADVANCED downloads `manage_amneziawg.sh` and `awg_common.sh` into a temporary folder, verifies each file with `minisign -P` and replaces the working copies in `/root/awg/` only after a successful check. The block only accepts a pair of files from one release that is not older than the installed one, so a tampered latest-release pointer cannot roll the scripts back to older signed versions, and it prints the verified release number. The integrity line in README matches the download from the latest release.
- **What the installer changes and what `--uninstall` leaves.** README lists the main changes the way the code makes them now, including what `--yes` and `--no-tweaks` do. README and both install guides say what removal takes away and what stays on the server: dependency packages, swap, the module package on ARM, and the `/root/awg_uninstall_backup_*.tar.gz` archive with configs and private keys, which you have to delete yourself.
- **How to remove the cascade.** CASCADE gains a section with removal commands for both servers. CASCADE and WARP-RU warn that they have to be removed before `--uninstall`: the installer only stops `awg0` and deletes `/etc/amnezia` and `/root/awg` as a whole.
- **The English and Russian install guides are aligned.** The English one gains the hoster's emergency console, the table of four client files with two QR codes, `scp` from your own computer with `-P` for a non-standard port, a "What next" section and the unreachable-address item. The Russian one gains the reboot diagram and the related reading list.
- **Corrections in ADVANCED and README.** Containers: the installer stops inside them, and the LXC path is `amneziawg-go`. Moving to a new server: check the network interface, `ListenPort` and `AWG_ENDPOINT`. `regen` takes the address from `AWG_ENDPOINT`. The "Amnezia list" mode is no longer called split routing. The Russian install guide describes the switch to `noble` as conditional. The English install guide gives `I1 = <r 48>` for Tele2 (Krasnoyarsk). For a leaked config the guides advise `remove` and `add`: `regen` keeps the same keys. The hoster affiliate link is marked as one.
- **Issue templates.** The issue and Q&A templates ask for the client app, the network and the routing mode, show how to get the version and link the English FAQ.
- **Contributor documents.** ROADMAP, SIGNING_DESIGN, RELEASE_PROCESS, the PR template and CONTRIBUTING describe the process that runs now: signatures before the tag, `AWG_COMMON_VERSION` among the version places, the commit hygiene check.
- **`wgcf` is checked in the WARP guide.** The binary is downloaded under its release name, checked against `checksums.txt` from the same release and run only after that check passes; a `wgcf` left from an earlier run is removed first. The security section says what that check does not protect against.
- **How the installer picks the repository on newer Ubuntu.** README, both guides, ADVANCED, the support matrix and CONTRIBUTING describe the mechanism: the installer checks whether the PPA answers for the release codename and switches to `noble` only if it does not. Debian uses a fixed mapping: 12 -> `focal`, 13 -> `noble`.
- **Samples match what the code produces.** The `stats` output is rendered with the same functions as in `manage`; the diagnostic report table lists all 17 sections; the client configs in ADVANCED and CASCADE show two DNS servers and `0.0.0.0/0, ::/0`; the log lines for a prebuilt ARM module and the DKMS fallback, the Fail2Ban `backend`, the MTU in `vpn://` and the fact that `manage` derives `server_public.key` itself now follow the code.
- **Five ADVANCED FAQ questions get their own anchors** (yellow config, mobile data, iPhone, "third attempt", changing parameters); links from README and the guides that pointed at the top of the FAQ now go straight to the answer. The `diagnose --carrier` profile names are listed; `tmobile_us` is a historical name for the Russian T-Mobile (Moscow and region).
- **Smaller fixes.** The `add guest` command in README has the full path, links in the Russian install guide lead to the right sections, the English guide's long paragraph became a flag list, `AWG_BRANCH` goes after `sudo`, cascade step 1 warns about the two reboots, `--force` does not keep the WARP `PostUp` line, the threat model link points to `docs/SIGNING_DESIGN.md`, SECURITY names both language versions of the scripts and the release pipeline, and `signing/README.md` uses `scripts/sign-release.sh` and verification with the whole key.
- **The documentation guard checks more.** Cross-file and HTML links to anchors, equal anchor sets in the RU/EN pairs of ADVANCED, CASCADE and WARP-RU, the return of the old wording about modes 2 and 3, and the version placeholder in the discussion template.
...
- **Groundwork for the third-line path: the installer can tell whether the loaded kernel module understands the 3.1 parameters.** The check is not active in this version: `--protocol=3.1` is still refused at step 0 for the same reasons as before, and an ordinary install does not run it. It starts working when the 3.1 path opens. The line cannot be told from the module version string: the same string was seen on different builds. So the capability is probed: a header protection key and a padding range are set on a temporary interface and read back. Once the path opens, a second-line module will be named as such together with the way to update it, and a check that could not be made will be reported as such instead of a guess. The probe key is one-shot, never appears in a command line and never in the log, `--verbose` included, and the temporary interface is removed on every ordinary exit, a signal included; and when it cannot be removed, the installer names it or says how to find it, and how to take it away by hand.
...
- **The header protection key no longer reaches the `manage show` and `manage check` output or the installer diagnostic report when `WG_COLOR_MODE=always` is set.** With that variable `awg show` colours its labels even when the output is not a terminal, and the secrets filter did not recognise the `header protection key` line. The scripts now run the tools with `WG_COLOR_MODE=never`. Under the same variable the step 7 service check of the installer failed falsely, `check` reported the obfuscation parameters as not detected, and `diagnose` found no `jc`, `jmin` and `jmax`; that is fixed too.
- **The service status text in `manage check`, and on a failed `restore` or `restart`, no longer shows keys from `amneziawg-tools` messages.** `systemctl status` ends with the unit's journal lines, and with a malformed key in `awg0.conf` (a character lost in a hand edit, `PrivateKey` inside `[Peer]`) those carried the `amneziawg-tools` message with the key value: `Line unrecognized: ...` or `Key is not the correct length or format: ...`. This text used to go to the screen, to the `manage` log and into reports people post in issues. It now goes through the same secrets filter as the installer's diagnostic report; if the filter itself fails, `check` hides the status text and exits with an error, while `restore` and `restart` say the status is hidden and name the command to see it. With `--json` this text still goes to stderr, and in the plain mode `systemctl`'s own warnings stay on stderr too.
- **The secrets filter catches a key where a hand edit broke the line.** `amneziawg-tools` strip whitespace from an unrecognized line, so a missing `=`, a typo in the key name, a colon instead of `=` or the tail of a key wrapped onto the next line gave a `Line unrecognized: ...` message with the key still visible. Where the line still reads as `Name = value` for a known parameter (for example `RandomTrailers` outside its section), only the name now stays visible and the value is hidden; any other such line, a broken separator included, is hidden in full. On top of that, a value in `` `...' `` quotes that looks like a key as a whole is hidden in any message, for example `Unable to parse Jc: ...` when a key landed in the wrong field, and in `Unable to parse IP address: ...` a value that cannot be an address is hidden: this is how `amneziawg-tools` answer a key pasted into `AllowedIPs`. Addresses, endpoints, ports and numbers in these messages stay visible. This applies to the installer's diagnostic report, `manage check`, `show`, `diagnose` and the service status text.
...
- **`add`, `regen`, `modify`, `vpn://` issuing and a repeated installer run no longer produce broken profiles when `HeaderProtectionKey` is written into the server config.** The header protection key is two-sided: a profile without it does not connect. Previously, on an installation marked as generation 2.0 whose `awg0.conf` had the key added by hand, these commands silently issued client profiles without the key. They now refuse and name the reason: the `AWG_PROTOCOL` marker in `awgsetup_cfg.init` and the key in the config contradict each other. `modify` checks this before touching any client file, and the installer stops before rewriting the server config. The same refusal applies when a key exists and the marker cannot be read; without a key a broken marker only produces a warning in `manage` commands. Otherwise servers without `HeaderProtectionKey` work as before.

### Fixed

- **An install no longer stops with "Failed to hold amneziawg-dkms" while the hold is actually in place** (issue #285). The hold check runs on a kernel older than 6.7, Debian 12 for example, and on ARM with a prebuilt module on any kernel. After `apt-mark hold` the installer compared the result with the `apt-mark showhold` list piped into `grep`, and on some systems it refused with the hold in place: in the report from the issue dpkg showed `hold ok not-installed` and a manual `apt-mark showhold` listed the package. One mechanism for such a refusal reproduces: `apt-mark` writes the list line by line, `grep -q` quits at the first match, and if more held packages follow in the list the pipeline can count as failed under `pipefail`. Whether that was the cause on the reporter's host is not established yet. The check now looks where apt itself takes the hold from, the package selection in dpkg, and reads the `apt-mark showhold` list as a fallback, without a pipe. If neither shows the hold, the install still stops, because without it `amneziawg-tools` can pull the AmneziaWG 3.0 module from the PPA, bypassing the chosen path. Such a refusal now prints what `apt-mark hold` answered and the package status in dpkg, not only a general hint to check the apt lock.
...
- **The trace of the installer with `--verbose` and of `manage` under `bash -x` no longer shows keys.** It used to show server and client private keys, `PresharedKey` values of carried-over peers, the header protection key and `vpn://` links: while generating keys, rendering the server config, creating the default clients, and reading and checking the server config. Such functions now run entirely with tracing off, and tracing is turned back on after them. The same applies to `manage` run under `bash -x` (`list`, `stats`, `check`, `diagnose`, `modify`, `add`, `regen`, `remove` and the service status printed when `restore` or `restart` fails), where keys also leaked while regenerating clients and applying the config. A value passed directly on the command line is still shown by the trace before the function is entered. The service status text itself (`check`, `restore`, `restart`) is still printed unmasked; only its copy in the trace is gone.
...
- **Tag lengths in `I1`-`I5` are now checked before profiles and the server config are issued.** `amneziawg-go` accepts `<r>`, `<rc>` and `<rd>` with a negative length and crashes when it builds the packet, while the kernel module, when a `<b>` stands next to it, accepts it without an error and writes past the end of the buffer (upstream `amneziawg-linux-kernel-module#233`). The installer itself never generates such a value, but it could reach `awg0.conf` through a manual edit or someone else's config, and `regen` would hand it to every client. Issuing profiles and the server config with such a value now stops with a clear error, `modify` does not reissue a profile whose `.conf` already carries such a value, and `restore` refuses such a backup and rolls back. Only what is proven dangerous is refused: a negative length, a length that is not a number or has more than nine significant digits, and a total above 65535 bytes in one value. The value is read the way the implementations apply it: a comment after `#` dropped, an unterminated last tag parsed too; `restore` also checks every `I1`-`I5` line regardless of key and section case, as `amneziawg-tools` read them, and `modify` checks the client `.conf` the same way. A trailing comment is now also dropped when `awg0.conf` is read, so a `# ...` tail no longer reaches client profiles and `vpn://`. Unknown tags and other malformed input are still rejected by the implementations themselves. The boundary: the check guards issuing profiles, generating the server config and `restore`. Paths that only start or reload the service apply the `awg0.conf` already in place without it, for example `systemctl restart`, `manage restart`, `repair-module`, the start at boot and the apply after `remove` or after expired clients are removed.
- **After `modify`, QR and `vpn://` files match the edited `.conf` or are absent.** After editing a client `.conf`, the command rebuilds `NAME.png`, `NAME.vpnuri` and `NAME.vpnuri.png`. When one of them could not be rebuilt, for example without `qrencode` or the `perl` modules, the previous file stayed in place with the previous values while the command reported success, so a consumer such as a bot could hand it out as current. With the `I1`-`I5` check in this same release, a server with a refused value and a clean client `.conf` became one more such case: the `.conf` edit goes through, but `vpn://` cannot be built. These files are now removed before the edit and rebuilt after it, so a failed or interrupted rebuild leaves a missing file rather than a stale copy; a `modify` whose own value check fails (for example an invalid DNS) leaves the files alone. The `--json` reply of `modify` gains the `qr` and `vpnuri` fields, as in `add`: a path when the file exists, `null` when it does not. If a previous file cannot be removed, `modify` fails and leaves the `.conf` unchanged; the files removed before it are rebuilt by repeating the `modify` once the cause is fixed.
