...
- `tests/test_boot_critical_packages.bats` (27 checks): asserts the order, not just the presence. `apt-mark manual` must come before the upgrade and the comparison after it but before the reboot; presence alone would not be enough, since the pre-existing `apt-mark hold` was present too and still did not help. The functional part lifts the real blocks out of the shipped installer rather than reimplementing them. Checked by feeding it deliberately broken installer versions: removing any of the three guards, moving the check off its last-action slot, downgrading its `die` to a warning, dropping a package from the list or loosening the dpkg status parsing all turn the suite red
