...
- **The full list of flags after which configs must be reissued.** Besides `--preset`, `--jc`, `--jmin` and `--jmax` these are `--mobile`, `--port` and `--endpoint`: they change the obfuscation parameters or the port and address in `Endpoint`. The mobile advice no longer suggests simply reinstalling a running server with `--mobile`.
