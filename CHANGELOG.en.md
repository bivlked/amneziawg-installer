...
- **`wgcf` is checked in the WARP guide.** The binary is downloaded under its release name, checked against `checksums.txt` from the same release and only then renamed. The security section says what that check does not protect against.
- **How the installer picks the repository on newer Ubuntu.** README, both guides, ADVANCED, the support matrix and CONTRIBUTING describe the mechanism: the installer checks whether the PPA has packages for the release codename and switches to `noble` only if it does not. Debian uses a fixed mapping: 12 -> `focal`, 13 -> `noble`.
- **Samples match what the code produces.** The `stats` output is rendered with the same functions as in `manage`; the diagnostic report table lists all 17 sections; the client configs in ADVANCED and CASCADE show two DNS servers and `0.0.0.0/0, ::/0`; the log lines for a prebuilt ARM module, the Fail2Ban `backend`, the MTU in `vpn://` and the fact that `manage` derives `server_public.key` itself now follow the code.
- **Five ADVANCED FAQ questions get their own anchors** (yellow config, mobile data, iPhone, "third attempt", changing parameters), and links from README and the guides go straight to the answer. The `diagnose --carrier` profile names are listed; `tmobile_us` is a historical name for the Russian T-Mobile (Moscow and region).
- **Smaller fixes.** The `add guest` command in README has the full path, the English guide's long paragraph became a flag list, `AWG_BRANCH` goes after `sudo`, cascade step 1 warns about the two reboots, `--force` does not keep the WARP `PostUp` line, the threat model link points to `docs/SIGNING_DESIGN.md`, SECURITY names both language versions of the scripts and the release pipeline, and `signing/README.md` uses `scripts/sign-release.sh` and verification with the whole key.
