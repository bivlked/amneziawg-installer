...
- **`add`, `regen`, `modify` and `vpn://` issuing no longer produce broken profiles when `HeaderProtectionKey` is written into the server config.** The header protection key is two-sided: a profile without it does not connect. Previously, on an installation marked as generation 2.0 whose `awg0.conf` had the key added by hand, these commands silently issued client profiles without the key. They now refuse and name the reason: the `AWG_PROTOCOL` marker in `awgsetup_cfg.init` and the key in the config contradict each other. `modify` checks this before touching any client file. Servers without `HeaderProtectionKey` are not affected.

### Fixed

- **`manage check` and `manage show` no longer print the header protection key.** `awg show` prints it in clear text, and `check` carried that output to the screen and the log. The key line is now hidden the same way as in the diagnostic report, and the rest of the output is unchanged. In `diagnose` the first error line of `awg show` goes through the same filter.
- **A hung `awg show` is named as a timeout again.** In `check` and `show` the exit status was lost, so a generic error was printed instead of "did not answer within 10 seconds".
- **The installer with `--verbose` no longer prints the header protection key while checking the config.** Tracing is switched off for the check and turned back on afterwards.
