...
- **An expiry stamp no longer attaches to a same-named permanent client.** After `restore` or a new `add` with the same name, a client could inherit the deadline of the previous one and be deleted by cron at that deadline. `add` and `restore` now drop such a stamp; if it cannot be removed, `add` refuses that client and `restore` rolls back. Stamps from the archive are restored as before, and `restore` now checks that they actually landed, rolling back otherwise. An archive without an `expiry/` directory had no deadlines in it, so clients restored from it lose their stamps.
