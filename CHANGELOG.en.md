...
- **`modify` no longer leaves stale QR and `vpn://` files behind.** After editing a client `.conf`, the command rebuilds `NAME.png`, `NAME.vpnuri` and `NAME.vpnuri.png`. When one of them could not be rebuilt, for example without `qrencode` or the `perl` modules, the previous file stayed in place with the previous values while the command reported success, so a consumer such as a bot could hand it out as current. With the `I1`-`I5` check in this same release, a server with a refused value became one more such case: the `.conf` edit goes through, but `vpn://` cannot be built. These files are now removed before the edit and rebuilt after it, so a failed or interrupted rebuild leaves a missing file rather than a stale copy; a value refused by validation keeps the files. The `--json` reply of `modify` gains the `qr` and `vpnuri` fields, as in `add`: a path when the file exists, `null` when it does not. If a previous file cannot be removed, `modify` fails and leaves the `.conf` unchanged.
