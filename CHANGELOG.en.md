...
- `tests/test_boot_critical_packages.bats` (16 checks): asserts the order, not just the presence. `apt-mark manual` must come before the upgrade and the comparison after it but before the reboot; presence alone would not be enough, since the pre-existing `apt-mark hold` was present too and still did not help. The functional part lifts the real blocks out of the shipped installer rather than reimplementing them. Verified against nine mutants: removing either guard, moving the check past the reboot, downgrading its `die` to a warning, or dropping a package from the list all turn the suite red
