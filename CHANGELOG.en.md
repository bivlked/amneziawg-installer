...
- **The secrets filter catches a key where a hand edit broke the line.** `amneziawg-tools` strip whitespace from an unrecognized line, so a missing `=`, a typo in the key name, a colon instead of `=` or the tail of a key wrapped onto the next line gave a `Line unrecognized: ...` message with the key still visible. Such a message is now hidden in full, while known keys still show their name. On top of that, a value in `` `...' `` quotes that looks like a key as a whole is hidden in any message, for example `Unable to parse Jc: ...` when a key landed in the wrong field. Addresses, endpoints, ports and numbers in these messages stay visible. This applies to the installer's diagnostic report, `manage check`, `show`, `diagnose` and the service status text.
