...
- **Step 7 no longer mistakes a working service for a broken one on a server with many clients.** The interface, port and obfuscation checks read their output through `| grep -q` under `pipefail`, and on long output a check could fail by mistake. The output is now read in full first.
...
- **An expiry stamp no longer attaches to a same-named permanent client.** After `restore` or a new `add` with the same name, a client could inherit the deadline of the previous one and be deleted by cron at that deadline. `add` and `restore` now drop such a stamp; if it cannot be removed, `add` refuses that client and `restore` rolls back. Stamps from the archive are restored as before. An archive without an `expiry/` directory had no deadlines in it, so clients restored from it lose their stamps.
