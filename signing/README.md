...
with the trusted comment `amneziawg-installer <TAG> <file>`, asks for the key
password once and refuses to run without a terminal. It replaced a hand-written loop that failed silently
without one; see [docs/RELEASE_PROCESS.md](../docs/RELEASE_PROCESS.md).

That trusted comment is not decoration. A signature proves that some bytes
...
Everything needed except the key is attached to each release:
...
The key in these commands is only as trustworthy as the place you copied it
from. Taken from this repository in the same session as the script, it gives no
protection against a compromise of the repository: an attacker able to replace
the script can replace its signature and the key as well. Pin the key once from
a source outside GitHub and pass it with `-P` afterwards; that is what turns the
